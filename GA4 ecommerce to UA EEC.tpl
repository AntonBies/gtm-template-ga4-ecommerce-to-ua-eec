...
if (ec.hasOwnProperty('items')) {
  ...
  if (ev === 'begin_checkout') {
    eec.checkout = {actionField: {step: 1}};
  ...
  if(ev === 'add_shipping_info') {
    eec.checkout = {actionField: {step: 2, option: ec.shipping_tier}};
    return {ecommerce: eec};
  }
  
  if(ev === 'add_payment_info') {
    eec.checkout = {actionField: {step: 3, option: ec.payment_type}};
    return {ecommerce: eec};
  }
